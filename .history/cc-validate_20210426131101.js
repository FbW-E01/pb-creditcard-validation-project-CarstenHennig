...
function validateCreditCard(ccNum) {
  ...
  changeToNumber = Number(ccNum.toString());

  ccNum[ccNum.length - 1] % 2
    ? console.log("Last digit is even")
    : console.log("Last digit is odd");

  if (typeof changeToNumber == "number") console.log("Its a number");

  for (var i = 2; i < ccNum.toString().length; i++) {
    ccSum += Number(ccNum.toString([i]));
    console.log(ccSum);
  }
  if (ccSum > 16) {
